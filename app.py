...
:root{
    ...
    --dark:#0B1028; --ink:#111827; --muted:#6B7280;
}
...
.block-container{ padding:1rem 1.7rem 2rem !important; max-width:1550px !important; }
...
.hero{
    ...
    color:white;
    background:linear-gradient(135deg,#0B1028 0%,#35156B 48%,#087EA4 100%);
    box-shadow:0 24px 65px rgba(49,46,129,.28);
    border:1px solid rgba(255,255,255,.18);
}
.hero:before{
    content:""; position:absolute; right:5%; top:-70px;
    width:230px; height:230px; border-radius:50%;
    background:linear-gradient(135deg,rgba(236,72,153,.72),rgba(6,182,212,.60));
    filter:blur(18px); opacity:.60;
}
...
.hero-badge{
    ...
    color:#EDE9FE; font-size:12px; font-weight:800;
...
.hero h1{
    ...
    margin:.65rem 0 .35rem; letter-spacing:-1px;
    background:linear-gradient(90deg,#FFFFFF,#DDD6FE,#A5F3FC);
    -webkit-background-clip:text; -webkit-text-fill-color:transparent;
}
.hero p{ color:#E0E7FF; max-width:900px; font-size:1rem; line-height:1.45; margin:0; }
...
.control-box h3{
    ...
    font-size:1rem; font-weight:900; margin:0 0 .75rem;
}
.control-box .stTextArea textarea{
    background:rgba(255,255,255,.08) !important;
    color:#fff !important; border:1px solid rgba(196,181,253,.30) !important;
    border-radius:18px !important; min-height:150px !important;
}
.control-box label, .control-box p, .control-box span{ color:#F8FAFC !important; }
.tag{
    display:inline-flex; align-items:center;
    background:linear-gradient(135deg,rgba(139,92,246,.28),rgba(6,182,212,.18));
    color:#F5F3FF; font-size:11px; font-weight:800;
    padding:6px 11px; border-radius:999px; margin:3px;
...
.main-card{
    ...
    box-shadow:0 20px 45px rgba(79,70,229,.10);
    backdrop-filter:blur(14px);
    margin-bottom:1rem;
}
.section-title{
    font-family:'Space Grotesk',sans-serif; font-size:1.18rem;
    font-weight:900; color:#111827; margin:.2rem 0 .75rem;
}
...
.metric-card{
    ...
    border:1px solid rgba(139,92,246,.16); border-radius:22px;
    padding:1.2rem 1rem; text-align:center;
    box-shadow:0 18px 42px rgba(79,70,229,.12);
}
.metric-card:before{ content:""; position:absolute; inset:0 0 auto 0; height:5px; background:linear-gradient(90deg,var(--purple),var(--pink),var(--cyan)); }
.metric-card .label{ font-size:11px; font-weight:900; color:#6D28D9; text-transform:uppercase; letter-spacing:1px; margin-bottom:8px; }
.metric-card .value{ font-family:'Space Grotesk',sans-serif; font-size:2rem; font-weight:900; color:#111827; line-height:1; }
.metric-card .sub{ font-size:12px; color:#6B7280; margin-top:6px; }
.candidate-card{
    background:rgba(255,255,255,.93); border:1px solid rgba(226,232,240,.95);
    border-radius:20px; padding:1rem 1.15rem; margin-bottom:12px;
    display:flex; align-items:center; gap:1.15rem;
    box-shadow:0 14px 35px rgba(15,23,42,.08);
    transition:transform .2s ease, box-shadow .2s ease, border-color .2s ease;
}
.candidate-card:hover{ transform:translateY(-3px); box-shadow:0 22px 50px rgba(79,70,229,.18); border-color:rgba(139,92,246,.35); }
.rank-badge{ width:44px; height:44px; border-radius:15px; display:flex; align-items:center; justify-content:center; font-weight:900; font-size:15px; flex-shrink:0; }
...
.stButton > button{ background:linear-gradient(135deg,#7C3AED 0%,#EC4899 55%,#06B6D4 100%) !important; color:white !important; border:0 !important; border-radius:18px !important; font-weight:900 !important; font-size:1rem !important; padding:.9rem 2rem !important; box-shadow:0 18px 40px rgba(124,58,237,.35) !important; }
.stButton > button:hover{ transform:translateY(-2px); box-shadow:0 24px 55px rgba(236,72,153,.35) !important; }
.stDownloadButton > button{ background:rgba(255,255,255,.9) !important; color:#111827 !important; border:1px solid rgba(139,92,246,.22) !important; border-radius:16px !important; font-weight:800 !important; }
.stAlert{ display:none !important; }
hr{ border-color:rgba(255,255,255,.18) !important; margin:1rem 0 !important; }
[data-testid="stFileUploader"] section{ border-radius:22px !important; border:1.5px dashed rgba(139,92,246,.35) !important; background:rgba(255,255,255,.72) !important; box-shadow:0 14px 35px rgba(79,70,229,.08); }
@media(max-width:900px){ .metric-row{ grid-template-columns:repeat(2,1fr); } .candidate-card{ align-items:flex-start; flex-direction:column; } .control-box{ position:relative; } }
@media(max-width:620px){ .metric-row{ grid-template-columns:1fr; } .hero{ padding:1.3rem; } }
</style>
...
Minimum 3 years experience. Strong problem solving skills required.""",
    ...
    if total != 100:
        ...
        st.markdown(f"""
        <div style='background:rgba(255,255,255,0.08);border-radius:12px;padding:9px;margin-top:5px'>
            ...
            <div style='display:flex;gap:8px;margin-top:6px;font-size:10px;color:#c4b5fd;flex-wrap:wrap'>
    ...
    st.markdown("---")
    top_n = st.slider("👥 Number of Candidates", 3, 20, 10)
