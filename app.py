...
.block-container{ padding:.75rem 1.15rem 1.3rem !important; max-width:1480px !important; }

.hero{
    position:relative; overflow:hidden; border-radius:20px;
    padding:1.05rem 1.35rem; margin:.15rem 0 .7rem; color:#FFFFFF;
    background:var(--primary); border:1px solid var(--primary-soft);
    box-shadow:0 18px 42px rgba(15,23,42,.14);
}
.hero>*{ position:relative; z-index:1; }
.hero-badge{
    display:inline-flex; align-items:center; gap:7px;
    background:rgba(6,182,212,.16); border:1px solid rgba(6,182,212,.32);
    color:#CFFAFE; font-size:11.5px; font-weight:800; padding:5px 13px; border-radius:999px;
}
.hero h1{
    font-family:'Space Grotesk',sans-serif; font-size:clamp(1.65rem,2.7vw,2.7rem);
    line-height:1.05; font-weight:900; margin:.5rem 0 .25rem; letter-spacing:-.8px; color:#FFFFFF;
}
.hero p{ color:#CBD5E1; max-width:980px; font-size:.93rem; line-height:1.4; margin:0; }

.panel-card{
    background:var(--card); border:1px solid var(--border); border-radius:20px;
    padding:.82rem .95rem; box-shadow:0 12px 30px rgba(15,23,42,.06); margin-bottom:.65rem;
}
.control-title{ font-family:'Space Grotesk',sans-serif; font-weight:900; font-size:1rem; color:var(--primary); margin:0 0 .6rem; }
.small-title{ font-family:'Space Grotesk',sans-serif; font-weight:900; font-size:.88rem; color:var(--primary); margin:0 0 .35rem; }

[data-testid="stTextArea"] textarea{
    border-radius:16px !important; border:1px solid var(--border) !important;
    background:#FFFFFF !important; box-shadow:inset 0 1px 2px rgba(15,23,42,.04) !important;
    color:var(--text) !important; font-size:.86rem !important; line-height:1.35 !important;
}
[data-testid="stTextArea"] textarea:focus{
    border-color:var(--accent) !important; box-shadow:0 0 0 3px rgba(6,182,212,.12) !important;
}

.tag{
    display:inline-flex; align-items:center; background:var(--accent-soft);
    color:#0E7490; font-size:10.3px; font-weight:800; padding:5px 9px;
    border-radius:999px; margin:3px; border:1px solid #BAE6FD;
}
.weight-mini{
    background:#F8FAFC; border:1px solid var(--border); border-radius:15px; padding:.46rem .62rem; margin-top:.08rem;
}
[data-testid="stSlider"]{ padding:.02rem 0 .06rem !important; }
[data-testid="stSlider"] label{ font-size:.76rem !important; color:#334155 !important; font-weight:800 !important; }
[data-testid="stSlider"] div[data-baseweb="slider"]{ margin-top:0 !important; }
.stSlider label, .stNumberInput label, .stFileUploader label{ font-weight:800 !important; color:#334155 !important; }

.upload-card{
    background:var(--card); border:1px solid var(--border); border-radius:20px;
    padding:.82rem .95rem .9rem; box-shadow:0 12px 30px rgba(15,23,42,.06); margin-bottom:.65rem;
}
[data-testid="stFileUploader"] section{
    border-radius:17px !important; border:1.5px dashed #94A3B8 !important;
    background:#F8FAFC !important; padding:.7rem !important;
}
[data-testid="stFileUploader"] section:hover{ border-color:var(--accent) !important; background:#F0FDFA !important; }
[data-testid="stFileUploader"] small{ color:var(--muted) !important; }

.run-card{
    background:var(--card); border:1px solid var(--border); border-radius:20px;
    padding:.72rem .95rem; box-shadow:0 12px 30px rgba(15,23,42,.06); margin-bottom:.75rem;
}
.stButton>button{
    background:var(--primary) !important; color:#FFFFFF !important;
    border:1px solid var(--primary) !important; border-radius:18px !important;
    font-weight:900 !important; font-size:1.05rem !important; letter-spacing:.3px !important;
    min-height:64px !important; padding:.85rem 1.5rem !important;
    box-shadow:0 16px 34px rgba(15,23,42,.18) !important; transition:all .18s ease !important;
}
.stButton>button:hover{
    background:#164E63 !important; border-color:#164E63 !important;
    transform:translateY(-1px); box-shadow:0 20px 42px rgba(22,78,99,.22) !important;
}
.stDownloadButton>button{
    ...
    font-weight:850 !important; min-height:48px !important;
}
.stDownloadButton>button:hover{ border-color:var(--accent) !important; color:#0E7490 !important; }

.section-title{ font-family:'Space Grotesk',sans-serif; font-size:1.03rem; font-weight:900; color:var(--primary); margin:.05rem 0 .52rem; }
.metric-row{ display:grid; grid-template-columns:repeat(4,1fr); gap:.85rem; margin:.75rem 0 1rem; }
.metric-card{
    background:#FFFFFF; border:1px solid var(--border); border-top:4px solid var(--accent);
    border-radius:18px; padding:1rem .85rem; text-align:center; box-shadow:0 12px 28px rgba(15,23,42,.06);
}
.metric-card .label{ font-size:10.5px; font-weight:900; color:#0E7490; text-transform:uppercase; letter-spacing:.85px; margin-bottom:7px; }
.metric-card .value{ font-family:'Space Grotesk',sans-serif; font-size:1.85rem; font-weight:900; color:var(--primary); line-height:1; }
.metric-card .sub{ font-size:11.5px; color:var(--muted); margin-top:6px; }

.candidate-card{
    background:#FFFFFF; border:1px solid var(--border); border-radius:17px;
    padding:.78rem .95rem; margin-bottom:9px; display:flex; align-items:center; gap:1rem;
    box-shadow:0 10px 24px rgba(15,23,42,.055); transition:transform .18s ease, box-shadow .18s ease, border-color .18s ease;
}
.candidate-card:hover{ transform:translateY(-2px); box-shadow:0 18px 36px rgba(15,23,42,.10); border-color:#BAE6FD; }

.recommend-card{
    background:#F8FAFC; border:1px solid #BAE6FD; border-left:5px solid var(--accent);
    border-radius:18px; padding:1rem 1.1rem; margin:.25rem 0 1rem; box-shadow:0 12px 28px rgba(15,23,42,.055);
}
.recommend-kicker{ font-size:10.5px; font-weight:900; color:#0E7490; text-transform:uppercase; letter-spacing:.8px; margin-bottom:6px; }
.recommend-title{ font-family:'Space Grotesk',sans-serif; color:var(--primary); font-size:1.22rem; font-weight:900; margin-bottom:4px; }
.recommend-body{ color:#475569; font-size:.88rem; line-height:1.45; font-weight:650; }
.why-line{ display:flex; align-items:center; flex-wrap:wrap; gap:5px; margin-top:7px; }
.why-label{ color:#0F172A; font-size:10.5px; font-weight:900; }
.why-chip{ background:#E0F7FB; color:#0E7490; border:1px solid #BAE6FD; border-radius:999px; padding:3px 7px; font-size:10px; font-weight:850; }
.why-pill{ background:#F1F5F9; color:#334155; border:1px solid #E2E8F0; border-radius:999px; padding:3px 7px; font-size:10px; font-weight:800; }

.rank-badge{ width:40px; height:40px; border-radius:13px; display:flex; align-items:center; justify-content:center; font-weight:900; font-size:14px; flex-shrink:0; }
...
@media(max-width:1120px){ .metric-row{ grid-template-columns:repeat(2,1fr); } }
...
<div class="hero">
    ...
    <p>AI-powered recruiter that ranks candidates the way a great recruiter would — not by matching keywords, but by actually understanding who fits the role.</p>
</div>
""", unsafe_allow_html=True)
...
def pretty_keyword(term):
    ...
    if term.lower() in upper_terms: return term.upper()
    if term.lower() == 'tensorflow': return 'TensorFlow'
    if term.lower() == 'scikit-learn': return 'scikit-learn'
    return term.title()
...
def get_candidate_reason_parts(row, job_keywords):
    ...
    for kw in job_keywords:
        ...
        if len(matched_terms) >= 5: break
    if not matched_terms:
        for term in str(row.get('skills','')).split():
            clean_term = term.strip('.,()').lower()
            if len(clean_term)>3 and clean_term not in matched_terms:
                matched_terms.append(clean_term)
            if len(matched_terms)>=4: break
    try: activity_value = int(row.get('activity_score',0))
    except: activity_value = row.get('activity_score',0)
    return matched_terms, f"{row.get('experience_years','')} yrs exp", f"Activity {activity_value}/100"

def make_reason_chips(matched_terms, exp_reason, act_reason):
    skill_chips = ''.join(f'<span class="why-chip">{html.escape(pretty_keyword(term))}</span>' for term in matched_terms[:5])
    return f'{skill_chips}<span class="why-pill">{html.escape(str(exp_reason))}</span><span class="why-pill">{html.escape(str(act_reason))}</span>'
...
def create_ranked_pdf_report(df_ranked, top_n, recommendation_reason, explanation_keywords, skill_weight, exp_weight, act_weight):
    ...
    with PdfPages(buffer) as pdf:
        ...
        fig.text(0.07,0.915,'Ranked candidate report — India Runs Hackathon 2026',fontsize=9.5,color='#64748B')
        ...
        for line in summary_lines:
            fig.text(0.07,y,line,fontsize=9,color='#0F172A'); y-=0.022
        ...
        for _,row in top_pdf.iterrows():
            table_rows.append([int(row.get('rank',0)),str(row.get('name',''))[:22],str(row.get('job_title',''))[:26],str(row.get('experience_years','')),f"{row.get('skill_match_score',0):.1f}",f"{row.get('activity_score',0):.0f}",f"{row.get('final_score',0):.1f}"])
        table_ax=fig.add_axes([0.06,0.26,0.88,0.38]); table_ax.axis('off')
        ...
        for (r,c),cell in table.get_celld().items():
            ...
            if r==0: cell.set_facecolor('#0F172A'); cell.set_text_props(color='white',fontweight='bold')
            else: cell.set_facecolor('#FFFFFF' if r%2 else '#F8FAFC'); cell.set_text_props(color='#0F172A')
        ...
        for _,row in df_ranked.head(min(5,top_n)).iterrows():
            ...
            terms=', '.join(pretty_keyword(t) for t in matched_terms[:4]) if matched_terms else 'Relevant skills'
            line=f"#{int(row.get('rank',0))} {row.get('name','')}: {terms}; {exp_reason}; {act_reason}; Final {row.get('final_score',0):.1f}"
            fig.text(0.07,y,line,fontsize=8.2,color='#334155'); y-=0.023
...
default_jd = """We are looking for a Senior AI/ML Engineer with strong Python skills.
...
with jd_col:
    ...
    job_description = st.text_area("JD", value=default_jd, height=160, label_visibility="collapsed", key="job_description_text")

with key_col:
    st.markdown('<div class="small-title">🔍 Keywords</div>', unsafe_allow_html=True)
    keywords = []
    if job_description:
        stop_words = {'and','or','the','a','an','is','are','we','for','with','in','of','to','be','experience','skills','required','preferred','minimum','years','strong','looking','problem','solving'}
...
with weight_col:
    ...
    if total != 100:
        ...
        st.markdown(f"""<div class='weight-mini'>
            ...
            <div style='display:flex;gap:9px;margin-top:6px;font-size:10.5px;color:#475569;flex-wrap:wrap;font-weight:800'>
                <span>Skill {skill_weight}%</span><span>Exp {exp_weight}%</span><span>Act {act_weight}%</span>
            </div></div>""", unsafe_allow_html=True)

with select_col:
    st.markdown('<div class="small-title">👥 Top N</div>', unsafe_allow_html=True)
    top_n = st.slider("N",3,20,10,label_visibility="collapsed")
...
st.markdown("Upload **candidates.jsonl** (full dataset) or **CSV files** (multiple allowed)", unsafe_allow_html=False)

up_col1, up_col2 = st.columns(2)
with up_col1:
    jsonl_file = st.file_uploader("Upload JSONL file (candidates.jsonl)", type=["jsonl","json"], label_visibility="visible")
with up_col2:
    csv_files = st.file_uploader("Upload CSV files (multiple)", type=["csv"], accept_multiple_files=True, label_visibility="visible")
...
def get_default_data():
    ...
    data = {
        ...
        'experience_years': [5,3,4,2,6,7,5,2,4,6,3,5,4,2,3,6,5,2,4,3,4,5,6,2,3,7,4,2,5,6,3,2,5,4,3,6,4,2,5,7,4,3,5,2,4,6,3,5,4,6],
        'job_title': [
            'ML Engineer','Java Developer','Data Scientist','Frontend Dev','AI Engineer',
            'Database Admin','Deep Learning Engineer','Software Dev','ML Engineer','Cloud Engineer',
            'NLP Engineer','Data Analyst','Computer Vision Engineer','Frontend Dev','RL Engineer',
            'BI Analyst','Data Scientist','Mobile Dev','ML Engineer','Security Engineer',
            'Data Analyst','ML Engineer','AI Researcher','Blockchain Dev','AutoML Engineer',
            'Data Engineer','AI Researcher','iOS Dev','Fraud Detection Engineer','NLP Engineer',
            'ML Engineer','UI Designer','NLP Engineer','Robotics Engineer','Multimodal AI Engineer',
            'Data Scientist','Computer Vision Engineer','Game Dev','Data Scientist','Search Engineer',
            'ML Engineer','Product Manager','MLOps Engineer','Network Engineer','Speech Engineer',
            'Business Analyst','ML Researcher','Cloud ML Engineer','ML Engineer','Full Stack Developer'
        ],
        'activity_score': np.random.randint(60,100,50).tolist(),
        'education': ['B.Tech CS','MCA','M.Tech AI','B.Tech IT','PhD AI','B.Tech CS','M.Tech ML','B.Tech CS','M.Tech DS','B.Tech CS','M.Tech NLP','MSc Stats','M.Tech AI','B.Tech IT','PhD ML','MBA Analytics','M.Tech DS','B.Tech IT','M.Tech CS','B.Tech CS','B.Tech CS','M.Tech CS','PhD AI','B.Tech CS','M.Tech AI','M.Tech DE','PhD AI','B.Tech IT','M.Tech CS','PhD NLP','M.Tech ML','B.Des','PhD NLP','B.Tech Robotics','PhD AI','MSc Stats','M.Tech CS','B.Tech CS','PhD Stats','M.Tech CS','M.Tech AI','MBA','M.Tech CS','B.Tech CS','M.Tech CS','MBA','PhD ML','M.Tech CS','M.Tech AI','B.Tech CS']
    }
...
if run:
    if total != 100:
        ...
        with st.spinner("🤖 Analyzing candidates — please wait..."):
            ...
            # ── CSV mode ──────────────────────────────────────
                ...
                if csv_files:
                    ...
                    if len(df) < 5:
                        st.warning(f"⚠️ Only {len(df)} candidates. Using default dataset.")
                        df = get_default_data()
                    else:
        ...
        st.success(f"✅ Ranking complete! {len(df_ranked):,} candidates scored" + (f", {honeypots:,} honeypots filtered." if honeypots else "."))

        st.markdown('<div class="panel-card">', unsafe_allow_html=True)
        top1 = df_ranked.iloc[0]
        st.markdown(f"""<div class="metric-row">
            <div class="metric-card"><div class="label">👑 Top Candidate</div><div class="value" style="font-size:1.1rem">{html.escape(str(top1['name']))}</div><div class="sub">{html.escape(str(top1.get('job_title','')))}</div></div>
            <div class="metric-card"><div class="label">🎯 Top Score</div><div class="value">{df_ranked['final_score'].max():.1f}</div><div class="sub">out of 100</div></div>
            <div class="metric-card"><div class="label">📊 Scored</div><div class="value">{len(df_ranked):,}</div><div class="sub">candidates</div></div>
        ...
        top1_terms, top1_exp, top1_act = get_candidate_reason_parts(top1, keywords)
        top1_terms_text = ', '.join(pretty_keyword(t) for t in top1_terms[:4]) if top1_terms else 'relevant role skills'
        recommendation_reason = (f"Highest final score ({top1['final_score']:.1f}/100), matched {top1_terms_text}, "
                                 f"{top1.get('experience_years','')} years experience, and {top1_act.lower()}.")
        ...
        for _, row in df_ranked.head(top_n).iterrows():
            ...
            score = row['final_score']; skill = row['skill_match_score']
        ...
        with c1:
            ...
            t10 = df_ranked.head(10)
            clrs = ['#0F172A' if i==0 else '#06B6D4' if i<3 else '#CBD5E1' for i in range(len(t10))]
            bars = ax1.barh(t10['name'],t10['final_score'],color=clrs,height=0.42)
            ...
            ax1.invert_yaxis(); ax1.set_xlim(0,100); ax1.tick_params(colors='#64748B',labelsize=6.8)
        ...
        with c2:
            ...
            ax2.tick_params(colors='#64748B',labelsize=6.8); ax2.grid(alpha=0.15,linewidth=0.7)
        ...
        pdf_report = create_ranked_pdf_report(df_ranked,top_n,recommendation_reason,keywords,skill_weight,exp_weight,act_weight)

        dl1,dl2,dl3 = st.columns(3)
        with dl1:
            st.download_button("⬇️ Download Full Ranked CSV", data=csv_out, file_name="full_ranked_candidates.csv", mime="text/csv", use_container_width=True)
        with dl2:
            st.download_button("⬇️ Download Submission CSV (Top 100)", data=sub_out, file_name="submission.csv", mime="text/csv", use_container_width=True)
        with dl3:
            st.download_button("📄 Download PDF Report", data=pdf_report, file_name="ranked_candidates_report.pdf", mime="application/pdf", use_container_width=True)

        st.markdown("<div style='text-align:center;padding:1.5rem 0 0.5rem;color:#64748B;font-size:12px'>🤖 Intelligent Candidate Discovery · India Runs Hackathon 2026 · Built by Irfan Shafi</div>", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
