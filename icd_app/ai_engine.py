...
Provider strategy: Groq is tried first (fast, generous free tier — 30 req/min),
falling back automatically to Google Gemini if Groq is unavailable, rate-limited
beyond retry, or its key isn't configured.
...
def active_provider() -> str:
    """Which provider will be used first, for display in the UI."""
    if _get_groq_key():
        return "Groq (primary)" + (" + Gemini fallback" if _get_gemini_key() else "")
    elif _get_gemini_key():
        return "Gemini (Groq key not set)"
...
def _call_json(prompt: str) -> dict:
    """
    Try Groq first (faster, more generous free tier). Fall back to Gemini
    if Groq isn't configured or fails after its own retries.
    """
    ...
    if groq_key:
        try:
            ...
            raise RuntimeError(f"Groq failed and no Gemini fallback configured: {groq_error}")

    if gemini_key:
        return _call_gemini(prompt)

    raise RuntimeError("No API key configured. Add GROQ_API_KEY and/or GEMINI_API_KEY.")
...
- Home: the command center and landing page. Shows live recruitment stats (resumes uploaded/analyzed,
  shortlisted/selected/rejected counts, interviews pending/completed, active job openings, average ATS
  score, average interview score, hiring success rate), a recent activity timeline, recruitment charts
  (hiring funnel, score distribution, top skills, education mix), the ranked shortlist with each
  candidate's overall score, a breakdown by skills/experience/education match, matched skills, identified
  gaps, a recruiter-style summary, and Selected/Rejected decision buttons. The ranking uses the
  recruiter's priority weighting, not a flat average.
- Interview: combines interview prep and tracking in one page. The "Prep & Schedule" tab generates
  ...
  lets the recruiter schedule an interview. The "Track Interviews" tab shows scheduled/completed/cancelled
  interviews and lets the recruiter submit a final interview score out of 100 for completed interviews —
  once submitted the score is permanently locked (shown with a lock icon) and cannot be edited, to keep
  scoring fair and consistent.
